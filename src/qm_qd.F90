...
enddo
traj%H0=0.d0     
do i=1,ctrl%nstates  !calculate the trace of hamiltonian
   traj%H0=traj%H0+real(traj%H_MCH_ss(i,i))  !no real will produce same results
enddo
traj%H0=traj%H0/ctrl%nstates
do i=1,ctrl%nstates
   traj%H_MCH_ss(i,i)=traj%H_MCH_ss(i,i)-traj%H0*dble(ctrl%trace) !only real part minus H0
enddo
...
!...calculate the trace of gradients
traj%G0=0.d0
...
!write(*,*)"overlap1=",((traj%overlaps_ss(i,j),j=1,ctrl%nstates),i=1,ctrl%nstates)
...
if (stat==0) then
   ...
   write(*,*)"traj%phases_s=",traj%phases_s
endif
if (traj%step==0) then
...
!...calculate NACddt for debugging
if(printlevel>2)then
  ...
  do iatom=1,ctrl%natom
     ...
     enddo
  enddo
  write(u_ddt,'(i7,1x,100(e23.12,1x))')traj%step,((traj%NACdt_ss(i,j),j=1,ctrl%nstates),i=1,ctrl%nstates)
endif
...
do istate=1,ctrl%nstates
   do jstate=1,ctrl%nstates
      if ( istate .eq. jstate ) then
         dentemp=0.5d0*( conjg( z(istate) ) * z(jstate)  - gw )
      else
         dentemp=0.5d0*( conjg( z(istate) ) * z(jstate) )
      endif
      do iatom=1,ctrl%natom
         do idir=1,3
            traj%grad_ad(iatom,idir)=traj%grad_ad(iatom,idir) + &
            traj%Gmatrix_ssqd(istate,jstate,iatom,idir)*dentemp
         enddo !idir
      enddo !iatom
   enddo !jstate
enddo !istate
...
!
subroutine get_Dmatrix(ctrl,traj)
use definitions
implicit none
type(ctrl_type) :: ctrl
type(trajectory_type) :: traj
integer::istate,jstate,initState
real*8,allocatable::Hel(:,:)
complex*8,allocatable::Udt(:,:)
!...ii is Imaginary unit

initState = traj%state_mch

allocate(Hel(ctrl%nstates,ctrl%nstates))
allocate(Udt(ctrl%nstates,ctrl%nstates))

Hel = traj%H_MCH_ss ! Current energies
...
select case (trim(ctrl%model))
  ...
  case ('morse')
    if(ctrl%nstates/=3)then
      write(*,*)"In Morse model, we only consider 3 states and two atoms"
      stop
    endif
    call get_model_morse(ctrl%nstates,ctrl%natom,traj%geom_ad,&
                        traj%H_MCH_ss,traj%Gmatrix_ssqd)
...
!diabatic hamiltonian; try to take the trace out
!H0=0.5d0*(traj%H_MCH_ss(1,1)+traj%H_MCH_ss(2,2)) 
!traj%H_MCH_ss(1,1)=traj%H_MCH_ss(1,1)-H0*dble(ctrl%trace)
!traj%H_MCH_ss(2,2)=traj%H_MCH_ss(2,2)-H0*dble(ctrl%trace)
...
!...calculate state-independent gradient in diabatic basis
...
!adiabatic hamiltonian; try to take the trace out
H0=0.5d0*(traj%H_MCH_ss(1,1)+traj%H_MCH_ss(2,2)) 
traj%H_MCH_ss(1,1)=traj%H_MCH_ss(1,1)-H0*dble(ctrl%trace)
traj%H_MCH_ss(2,2)=traj%H_MCH_ss(2,2)-H0*dble(ctrl%trace)
...
write(88,'(9e23.13)')((traj%Gmatrix_ssqd(istate,jstate,1,1),&
                       jstate=1,ctrl%nstates),istate=1,ctrl%nstates)
!...calculate state-independent gradient in adiabatic basis
traj%Gmatrix0=0.5d0*(traj%Gmatrix_ssqd(1,1,1,1)+traj%Gmatrix_ssqd(2,2,1,1))
!...seperate the state-independent gradient
do istate=1,2
   traj%Gmatrix_ssqd(istate,istate,1,1)=traj%Gmatrix_ssqd(istate,istate,1,1)&
