...
! ===========================================================
  ...
  subroutine Adjust_phases(traj,ctrl)
    ...
    !Zhou
    !write(*,*)"begin to adjust phase"
    !write(*,*)"traj%phases_found=",traj%phases_found
    !write(*,*)"ctrl%calc_nacdr=",ctrl%calc_nacdr
    !write(*,*)"ctrl%calc_overlap=",ctrl%calc_overlap
    !write(*,*)"ctrl%surf=",ctrl%surf
    !Wanghuai
...
!         traj%phases_s=scalarProd(:,1)
    ...
    do istate=1,ctrl%nstates
      ...
      !if (ctrl%calc_nacdr>=1) then
      if (ctrl%calc_nacdr>=0) then !Zhou
    ...
    !write(*,*)"overlap after phase=",((traj%overlaps_ss(istate,jstate),&
  ...
  !       call diagonalize_and_project(ctrl%nstates,traj%H_diag_ss,traj%U_ss,traj%U_old_ss)
    ...
    !...correct the phase of off-diagonal elements of overlap matrix
    !call correct_overlaps_ss(ctrl%nstates,traj%overlaps_ss,traj%phases_old_s,traj%phases_s)
    !Since the SHARC has corrected the bug in the interface file, we do not need
    !to correct anymore
...
! ===========================================================
  !...correct the sign problem of overlap matrix
  subroutine correct_overlaps_ss(nstates,overlaps_ss,phases_old_s,phases_s)
  implicit none
  integer,intent(in)::nstates
  complex*16,intent(inout)::overlaps_ss(nstates,nstates)
  complex*16,intent(in)::phases_old_s(nstates),phases_s(nstates)
  integer::istate,jstate
  real*8::phase_old_product,phase_product,sign_product

  do istate=1,nstates-1
     do jstate=istate+1,nstates
        phase_old_product=real(phases_old_s(istate))*real(phases_old_s(jstate))
        phase_product=real(phases_s(istate))*real(phases_s(jstate))
        write(*,*)"phase_old_product=",phase_old_product
        write(*,*)"phase_product=",phase_product
        sign_product=phase_old_product*phase_product
        write(*,*)"sign_product=",sign_product
        if(sign_product<0.d0)then
          overlaps_ss(istate,jstate)=overlaps_ss(istate,jstate)*(-1.d0)
          overlaps_ss(jstate,istate)=overlaps_ss(jstate,istate)*(-1.d0)
        endif
     enddo
  enddo
        
  end subroutine  correct_overlaps_ss
! ===========================================================
