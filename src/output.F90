...
#ifdef Model
  write(u,*)'Model System running'
#else
  write(u,*)'ab initio running'
#endif
  write(u,*) ''
...
endsubroutine
  ...
  ! =====================================================
  !Zhou
subroutine write_Dmatrix(traj,ctrl)
  use definitions
  use matrix
  implicit none
  type(trajectory_type) :: traj
  ...
  !write(u_den,'(I12)') traj%step
  !call matwrite(ctrl%nstates, Dmatrix, u_den, '  ', 'E21.13e3')

  !output density matrix
  write(u_den,'(i7,1x,50(e23.12e3,1x))')traj%step,((Dmatrix(i,j),&
        j=1,ctrl%nstates),i=1,ctrl%nstates)

  !output mapping variables
  write(u_map,'(i5,1x,20(f16.8,1x))')traj%step,z

  !output force
  write(u_gma,'(i5,1x,300(f16.8,1x))')traj%step,((traj%grad_ad(i,j),j=1,3),i=1,ctrl%natom)

  !output phases of eigenvectors
  write(u_pha,'(i7,1x,50(f10.5,1x))')traj%step,(traj%phases_s(i),i=1,ctrl%nstates)

  !output overlap matrix
  if(traj%step==0)return
  write(u_over,'(i7,1x,50(e23.12e3,1x))')traj%step,((traj%overlaps_ss(i,j),&
        j=1,ctrl%nstates),i=1,ctrl%nstates)

end subroutine write_Dmatrix
  ! =====================================================
