...
! =================================================================== !
  ...
  ! log file, list file, data file and geometry file
    ...
    ! make new files if no restart
      ...
      !Zhou,density matrix
      open(unit=u_den, file='density.dat', status='replace', position='rewind', iostat=io)
      if (io/=0) then
        write(0,*) 'Could not create output file "density.dat"'
        stop 
      endif
      open(unit=u_over, file='overlap.dat', status='replace', position='rewind', iostat=io)
      if (io/=0) then
        write(0,*) 'Could not create output file "overlap.dat"'
        stop 
      endif
      open(unit=u_map, file='map.dat', status='replace', position='rewind', iostat=io)
      if (io/=0) then
        write(0,*) 'Could not create output file "map.dat"'
        stop 
      endif
      write(u_map,*)"# step, qF, pF, qB, pB"
      open(unit=u_gma, file='Gmat.dat', status='replace', position='rewind', iostat=io)
      if (io/=0) then
        write(0,*) 'Could not create output file "Gmat.dat"'
        stop 
      endif
      open(unit=u_pha, file='phase.dat', status='replace', position='rewind', iostat=io)
      if (io/=0) then
        write(0,*) 'Could not create output file "phase.dat"'
        stop 
      endif
      open(unit=u_ddt, file='NACdt.dat', status='replace', position='rewind', iostat=io)
      if (io/=0) then
        write(0,*) 'Could not create output file "NACdt.dat"'
        stop 
      endif
      write(u_ddt,*)"#Only print out NACddt elements for debugging if printlevel>2"
      !Wanghuai
    endif
...
!       endif
  ...
  ! cwd, ezero, scalingfactor, printlevel, RNGseed, dampeddyn, decoherence, decoherence_scheme, decoherence_alpha
    ...
    !Zhou,Model system we use
    line=get_value_from_key('model',io)
    if (io==0) then
       ctrl%model=trim(line)
       !write(*,*)"ctrl%model=",ctrl%model
       select case (trim(ctrl%model))
         case ('tully2')
            write(*,*)"#You choose Tully2 model."
         case ('morse')
            write(*,*)"#You choose Morse model."
       end select
    endif
    line=get_value_from_key('focused',io)
    if (io==0) then
      read(line,*)ctrl%init_cond  !=0 or 1
      !=0 sample initial condition
      !=1 focused condition
    else
      ctrl%init_cond=1   !focused condition
    endif
    write(*,*)"ctrl%init_cond=",ctrl%init_cond
    if(ctrl%init_cond/=0.and.ctrl%init_cond/=1)then
       stop "init_cond we do not know."
    endif 
    line=get_value_from_key('trace',io)
    if (io==0) then
      read(line,*)ctrl%trace  !=0 or 1
      !=0 no trace out
      !=1 take the trace out
    else
      ctrl%trace=1   !take the trace out
    endif
    write(*,*)"ctrl%trace=",ctrl%trace
    if(ctrl%trace/=0.and.ctrl%trace/=1)then
       stop "trace we do not know."
    endif 
    line=get_value_from_key('integrator',io)
    if (io==0) then
      read(line,*)ctrl%integrator  !=1 or 2
      !=1 Verlet
      !=2 Runge-Kutta 4
    else
      ctrl%integrator=1   !default: Verlet
    endif
    write(*,*)"integrator=",ctrl%integrator
    if(ctrl%integrator/=1.and.ctrl%integrator/=2)then
       stop "integrator method we do not know."
    endif 
    !Wanghuai
