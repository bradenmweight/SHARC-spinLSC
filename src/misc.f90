...
!> Checks whether the file "STOP" exists in the CWD
  logical function check_stop(cwd)
   ...
   !Zhou
  !...use Metropolis algorithm to generate gaussian random number
  subroutine random_gauss(N,mean,std,rand)
  implicit none
  integer,intent(in)::N
  ...
  do i=1,N
     findrand=.false.
     do while(.not.findrand)
       call random_number(x)
       x=2.d0*(x-0.5d0)    ![-1,1]
       x=mean+5.d0*std*x   ![mean-5*std,mean+5*std]
       call random_number(y)
       call get_gauss_prob(mean,std,x,prob)
       if(prob>y)then
         rand(i)=x
         findrand=.true.   !find one random # successfully
       endif
     enddo
  enddo
  return
  ...
  subroutine get_gauss_prob(mean,std,x,prob)
  implicit none
  real*8,intent(in)::mean,std,x
  real*8,intent(out)::prob
  real*8::pi=3.141592653589793d0
  real*8::std2

  std2=std*std
  prob=exp(-(x-mean)**2/2.d0/std2)/sqrt(2.d0*pi*std2)

  end subroutine get_gauss_prob
  !Wanghuai
