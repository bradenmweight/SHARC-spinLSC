...
!Zhou
  real*8,allocatable::Gmatrix_ssqd(:,:,:,:)   !< Cartesian gradient in a.u (hartree/bohr) of the current state in QD basis
  real*8::Gmatrix0 !state-independent gradient, only used for model system
  real*8::H0  !state-independent potential
  real*8,allocatable::G0(:,:) !state-independent gradients
!Wanghuai
...
!   real*8 :: diagonalize_degeneracy_diff=1.d-9     ! Energy difference threshold for treating states as degenerate                                in hartree
  ...
  !Zhou,Model stystem
  character(len=20)::model  !Model we use
  integer::init_cond   !used to control the initial condition of mapping variables
  integer::trace       !used to control trace out or not
  integer::integrator  !integrator method to propagate mapping variables
  !Wanghuai
...
!...Zhou,mapping variables in QD basis
!real*8,allocatable::qF(:),pF(:),qB(:),pB(:)
!real*8,allocatable::qFd(:),pFd(:),qBd(:),pBd(:) !diabatic basis
!real*8::qF0,pF0,qB0,pB0

!!!! SPIN MAPPING VARIABLES, BMW
complex*16,allocatable::z(:)
complex*16,allocatable::zd(:) !diabatic basis
complex*16,allocatable::z0(:) ! Initial states
real*8::gw ! ZPE Parameter


complex*16,allocatable::Dmatrix(:,:)
integer,parameter::u_den=18  !for population (density matrix)
integer,parameter::u_over=19 !for overlap matrix
integer,parameter::u_map=20  !for mapping variables
integer,parameter::u_gma=21  !for G matrix
integer,parameter::u_pha=22  !for phases
integer,parameter::u_ddt=23  !for NACddt
!...Wanghuai
...
! =========================================================== !
    ...
    subroutine allocate_traj(traj,ctrl)
      ...
      !Zhou
      allocate(traj%Gmatrix_ssqd(nstates,nstates,natom,3),stat=status)
      if (status/=0) stop 'Could not allocate Gmatrix_ssqd'
      traj%Gmatrix_ssqd=-123.d0
   
      allocate(traj%G0(natom,3),stat=status)
      if (status/=0) stop 'Could not allocate G0'
      traj%G0=-123.d0
      !Wanghuai
...
!Zhou
    write(u,'(A20,1X,L1)') 'Gmatrix_ssqd',    allocated(traj%Gmatrix_ssqd    )
!Wanghuai
